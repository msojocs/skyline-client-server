...
cmake --build "$root_dir/build" --config Release --target skylineClient --

mkdir -p "$root_dir/tmp/build"

mv "$root_dir/packages/native/build/skylineClient.node" "$root_dir/tmp/build/skyline-skylineClient-linux-$arch-$tag.node"